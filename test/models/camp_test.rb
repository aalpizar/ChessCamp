...
class CampTest < ActiveSupport::TestCase
  ...
  context "Within context" do
    ...
    should "verify that the camp's curriculum is active in the system" do
      ...
      bad_camp = FactoryGirl.build(:camp, curriculum: @smithmorra, location: @cmu, start_date: Date.new(2014,8,1), end_date: Date.new(2014,8,5))
      deny bad_camp.valid?
      # test the nonexistent curriculum
      gambit = FactoryGirl.build(:curriculum, name: "King's Gambit")
      gambit_camp = FactoryGirl.build(:camp, curriculum: gambit, location: @cmu, start_date: Date.new(2014,8,1), end_date: Date.new(2014,8,5))
    ...
    should "verify that the camp's location is active in the system" do
      ...
      bad_camp = FactoryGirl.build(:camp, curriculum: @tactics, location: @sqhill, start_date: Date.new(2014,8,1), end_date: Date.new(2014,8,5))
      deny bad_camp.valid?
      delete_inactive_locations
      # test the nonexistent location
      bhill = FactoryGirl.build(:location, name: "Blueberry Hill")
      bhill_camp = FactoryGirl.build(:camp, curriculum: @tactics, location: bhill, start_date: Date.new(2014,8,1), end_date: Date.new(2014,8,5))
    ...
    should "shows that a camp with same date and time slot but different location can be created" do
      @ok_camp = FactoryGirl.build(:camp, curriculum: @tactics, location: @north, start_date: Date.new(2014,7,21), end_date: Date.new(2014,7,25), time_slot: 'am')
      assert @ok_camp.valid?
    end

    should "shows that a duplicate camp (same date, time and location) cannot be created" do
      @bad_camp = FactoryGirl.build(:camp, curriculum: @tactics, location: @cmu, start_date: Date.new(2014,7,21), end_date: Date.new(2014,7,25), time_slot: 'am')
      deny @bad_camp.valid?
    end
    ...
    should "have a method for finding active camps that still have openings" do
      ...
      small_camp = FactoryGirl.create(:camp, curriculum: @endgames, start_date: Date.new(2014,8,21), end_date: Date.new(2014,8,26), time_slot: "pm", location: @cmu, max_students: 2)
